...
## 思维导图视口约定（v2 起）
- 默认只展开到第一层（`collapsed = depth >= 1`），保证首屏 fitView 后节点文字 ≥8px 可读；🎲 随机路径会自动展开途经层。
- 触屏双指捏合缩放（touchmove preventDefault + 以两指中点为中心）；单指平移走 pointer 事件，双指时强制结束拖拽避免手势打架。
- resize 自适应带 25% 阈值 + 200ms 防抖：窗口拖拽/横竖屏才重新 fitView，手机地址栏收放的小抖不重置用户视图。
